...
public:
	...
	void restart(int checkCont);

	float Picking(int x,int y, int mode, bool checkCont);
	void sameVector(int rotIndx, int myIndx, float x, float y);
	void changeLines();
	void changeRotSize();
	void rotAround(int rotIndx, int myIndx, float x, float y);
	void Resize(int width,int hight);

	int getClosestOct(float newX, float newY);
